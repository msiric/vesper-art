...
const ArtworkPanel = ({ type, fixed }) => {
  ...
  const renderArtwork = (artwork, loading, key) => (
    <ArtworkCard
      key={key}
      artwork={artwork}
      type={type}
  ...
  return (
    <Box className={classes.container}>
      ...
      >
        ...
        >
          {determineLoadingState(loading, limit, elements).map((artwork, index) =>
            renderArtwork(artwork, loading, artwork.id || `loading-${index}`)
          )}
          {determineFetchingState(fetching, limit).map((artwork, index) =>
            renderArtwork(artwork, fetching, `fetching-${index}`)
          )}
        </Masonry>
      </InfiniteList>
    </Box>
