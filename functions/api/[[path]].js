...
export async function onRequest({ request, env }) {
  const json = (status, message) => Response.json({ message, expose: true }, { status, headers: { 'Cache-Control': 'no-store' } });
  ...
  if (Number(request.headers.get('Content-Length') || 0) > 2200000) return json(413, 'Request is too large');
  ...
  try {
    ...
    if (!['GET', 'OPTIONS'].includes(request.method) && request.body) {
      ...
      for (;;) {
        ...
        if (size > 2200000) { await reader.cancel(); return json(413, 'Request is too large'); }
        chunks.push(value);
      }
